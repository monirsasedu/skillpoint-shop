...
function CopyableNumber({ label, number, color }: { label: string; number: string; color: string }) {
  ...
  return (
    <div className="flex items-center justify-between rounded-lg border border-gray-200 bg-gray-50 p-3 shadow-sm transition-all hover:border-brand dark:border-dark-400 dark:bg-dark-300 dark:hover:border-brand-dark">
      ...
      <button
        ...
        className={cn(
          ...
          copied
            ...
            : "bg-gray-200 text-dark-600 hover:bg-brand hover:text-white dark:bg-dark-600 dark:text-light-400 dark:hover:bg-brand-dark dark:hover:text-white"
        )}
      >
        {copied ? (
          <>
...
function ManualPaymentInstructionsCard() {
  ...
  return (
    <div className="rounded-lg bg-white p-6 dark:bg-dark-200 border border-gray-200 dark:border-[#434343] space-y-4 shadow-sm">
      {options?.manualPaymentDescription && (
        <p className="text-sm text-dark-600 dark:text-light-400 whitespace-pre-line leading-relaxed">
          {options.manualPaymentDescription}
        </p>
      )}
      ...
      <div className="mx-auto w-full max-w-screen-lg">
        <div className="relative overflow-hidden rounded">
          ...
          <div className="bg-light px-6 pb-12 pt-9 dark:bg-dark-200 lg:px-8">
            ...
            {order?.payment_gateway?.toUpperCase() === 'MANUAL_PAYMENT' &&
              order?.payment_status !== PaymentStatus.SUCCESS && (
                <div className="mt-12">
                  <h2 className="mb-6 text-base font-semibold dark:text-white">
                    Manual Payment Instructions
                  </h2>
                  <ManualPaymentInstructionsCard />
                </div>
              )}
